...
export const createSplit = async (req, res) => {
  ...
  try {
    ...
    if (transactions && transactions.length > 0) {
      ...
      // 3️⃣ Update split with settlements
...
// @desc Get participant IDs of a split
// @route GET /api/splits/:id/participants
// @access Private

export const getSplitParticipants = async (req, res) => {
  const splitId = req.params.id;
  ...
  try {
    const split = await Split.findById(splitId).select("participants").lean();

    if (!split) {
      return res.status(404).json({ message: "Split not found" });
    }

    res.json({ participantIds: split.participants });
  } catch (error) {
