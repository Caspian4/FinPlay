...
export default function Splits() {
  const API_URL = "http://localhost:3000/api/splits";
  ...
  // fetch all users to resolve usernames → IDs
  useEffect(() => {
    if (!token) return;
...
async function handleSettleSettlement(settlement) {
  ...
  try {
    ...
    if (remaining.length === 0) {
      // ✅ All settled, generate Split Card
...
function CrewRow({ member, splitEqually, updateCrewMember, removeCrewMember, canRemove }) {
  return (
    <div className="crew-row">
      ...
      <input
        type="text"
        value={member.username}   // ← use username instead of name
