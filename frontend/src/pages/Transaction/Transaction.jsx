...
if (!existingTransactionCard && rarity !== "common") {
  await axios.post(
    "http://localhost:3000/api/cards",
    {
      userId: userInfo._id,
      kind,
      title: kind === "income" ? `Received ₹${amount}` : `Spent ₹${amount}`,
      subtitle,
      value: amount,
      rarity,
      level: 1,
    ...
    if (totalRoundUp > 0) {
      ...
      if (existingRoundUpCard) {
        ...
        await axios.patch(
          `http://localhost:3000/api/cards/${existingRoundUpCard._id}`,
          {
            title: `Total Round-up ₹${totalRoundUp.toFixed(2)}`,
            value: totalRoundUp,
    ...
    const getTotalRoundUp = () => {
        ...
        return Number(total.toFixed(2)); // ✅ always 2 decimals
    };
                ...
                <div className="summary-cards">
                    <div className="card card2 p-4 text-center income">
                        ...
                        <div >
                            ...
                            <div className="amount">₹{getTotalByType('income').toLocaleString()}</div>
                        </div>
                    </div>
                    <div className="card card2  p-4 text-center expense">
                        ...
                        <div >
                            ...
                            <div className="amount">₹{getTotalByType('expense').toLocaleString()}</div>
                        </div>
                    </div>
                    <div className="card card2 p-4 text-center balance">
                        ...
                        <div >
                            ...
                            <div className="amount">
                                ₹{(getTotalByType('income') - getTotalByType('expense')).toLocaleString()}
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <TabsContent value="transactions" className="space-y-6">
                            ...
                            <ArchiveFilters
                                ...
                                categories={[
                                    ...
                                    { value: "Trade Profit", label: "Trade Profit", icon: "⚓" },
                                    {
                                        value: "Storm Damage",
                                        label: "Storm Damage",
                                        icon: "⛈️",
                                    },
                                    { value: "Cannon Fire", label: "Cannon Fire", icon: "💥" },
                                    {
                                        value: "Ship Repairs",
                                        label: "Ship Repairs",
                                        icon: "🔧",
                                    },
                                    { value: "Crew Wages", label: "Crew Wages", icon: "👨‍✈️" },
                                ]}
                                ...
                                <CardContent>
                                    <div>
                                        {getFilteredTransactions().map((transaction) => (
                                            <div
                                                key={transaction._id}   // ✅ use _id, not id
